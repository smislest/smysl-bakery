    ...
    const fetchFAQ = async () => {
      try {
        ...
        if (response.ok) {
          ...
          // Fallback to hardcoded data if Directus is unavailable
          setFaqData([
            {
        ...
        setFaqData([
          {
            question: "Что такое глютен и почему он вреден?",
            answer: "Глютен — это белок, содержащийся в пшенице, ржи и ячмене. Он может вызывать проблемы у людей с целиакией, непереносимостью глютена или чувствительностью к нему."
          }
        ]);
      } finally {
        setIsLoading(false);
